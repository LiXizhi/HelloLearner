...
2. Empties and rebuilds `dist/`.
3. Generates the hashed release HTML and updates the tutor skill URL in the sibling `../AIChat/AIChat_skills.json`. This modifies AIChat source even when you only run `npm run build`; it does not upload either application.

The standalone AIChat engine URL is selected at runtime: `127.0.0.1` / `localhost` use `../AIChat/AIChat.html` on the same origin and port; other hosts use `https://keepwork.com/api/raw/maisi/maisi/webgames/tools/AIChat/release/AIChat_v1.html`. Builds preserve this selection.
...
4. Updates the `local-language-learner` URL in `../AIChat/AIChat_skills.json` to `<baseHref>SKILL.md`, preserving other entries and catalog formatting. The sibling catalog must exist and contain exactly one matching skill; otherwise generation fails.

Upload HelloLearner successfully before rebuilding and publishing AIChat. The catalog update changes AIChat source only; its already-published release is not updated automatically. The tutor entry retains `"devUrl": "../HelloLearner/SKILL.md"`: AIChat opened on `127.0.0.1` uses this development path relative to its page URL, ignoring any CDN `<base>` for this override. Other hosts (including `localhost`) use the published `url`. Release generation updates only `url`, preserving `devUrl`. The lesson-planner catalog entry is not changed by this step.
