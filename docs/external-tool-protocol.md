...
| **Standalone** (top-level) | a hidden child iframe mounting `../AIChat/AIChat.html` | `sdk.personalPageStore` |
...
Exactly five commands are allowed, each with a `snake_case` alias accepted on the wire:
...
The host binds these to its current workspace. Paths are always `.hellolearner/profile.json`, `.hellolearner/progress.json`, or `.hellolearner/settings.json`. Default request timeout is 8 s; a timeout surfaces a save warning and the records stay in memory.
