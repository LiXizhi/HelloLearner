...
import { updateTutorSkillUrl } from './syncSkillCatalog.mjs';
...
html = html.replace(/<head\b[^>]*>/i, (headTag) => `${headTag}\n  <base href="${baseHref}">`);
...
const catalogPath = path.resolve(projectRoot, '../AIChat/AIChat_skills.json');
const catalogSource = fs.readFileSync(catalogPath, 'utf8');
const updatedCatalog = updateTutorSkillUrl(catalogSource, baseHref);
if (updatedCatalog !== catalogSource) fs.writeFileSync(catalogPath, updatedCatalog);
console.log(`[generateRelease] AIChat tutor skill URL: ${new URL('SKILL.md', baseHref).href}`);
